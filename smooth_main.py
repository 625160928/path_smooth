...
"""
def smooth(x,y,theta,method='poly',arg=None):
    ...
    if method=='bspline':
        new_route_x,new_route_y,new_route_theta=BSplineSmooth().smooth(x,y,theta,k=arg)
...
路径优化
输入：三个list
输出：将路径b样条曲线优化并提供x,y,theta,曲率信息
"""
def route_opt(x, y, theta):
    len_number=5

    new_route_x,new_route_y,new_route_theta=BSplineSmooth().smooth(x,y,theta,5)
    new_route_theta=[]
    new_route_cure=[]

    for i in range(len(new_route_x)-len_number+1):
        if i==0:
            new_route_theta+=PolynomialSmooth().get_theta(new_route_x[i:i+len_number],new_route_y[i:i+len_number])[0:3]
            new_route_cure+=PolynomialSmooth().get_cure(new_route_x[i:i+len_number],new_route_y[i:i+len_number])[0:3]
        elif i==len(new_route_x)-len_number-1:
            new_route_theta+=PolynomialSmooth().get_theta(new_route_x[i:i+len_number],new_route_y[i:i+len_number])[2:5]
            new_route_cure+=PolynomialSmooth().get_cure(new_route_x[i:i+len_number],new_route_y[i:i+len_number])[2:5]
        else:
            new_route_theta.append(PolynomialSmooth().get_theta(new_route_x[i:i+len_number],new_route_y[i:i+len_number])[2])
            new_route_cure.append(PolynomialSmooth().get_cure(new_route_x[i:i+len_number],new_route_y[i:i+len_number])[2])

    return new_route_x,new_route_y,new_route_theta,new_route_cure



if __name__ == '__main__':
    x = np.arange(1, 18, 1)
    y = np.array([4.00, 6.40, 8.00, 8.80, 9.22, 9.50, 9.70, 9.86, 10.00, 10.20,9, 10.32, 10.42, 10.50, 10.55, 10.58, 10.60])
    theta = np.array([4.00, 6.40, 8.00, 8.80, 9.22, 9.50, 9.70, 9.86, 10.00,9, 10.20, 10.32, 10.42, 10.50, 10.55, 10.58, 10.60])
